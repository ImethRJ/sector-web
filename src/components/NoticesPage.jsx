...
const NoticesPage = () => {
    ...
    const { notices } = useData(); // Get data from context

    const categories = ['All', 'Academic', 'Exam', 'Holiday', 'General'];

    // Remove the old 'const notices = [...]' array
    ...
    return (
        <section id="Notices" className="py-24 bg-slate-50 px-6 min-h-screen">
            <div className="max-w-[1200px] mx-auto">
                ...
                {/* Header - Consistent with TeachersHome */}
                <div className="text-center mb-16">
                    <h2 className="text-4xl font-black text-[#1a237e] mb-4 uppercase">Important Notices</h2>
                    <p className="text-gray-500 max-w-2xl mx-auto">Stay updated with the latest announcements, event schedules, and academic news from Sector Institute.</p>
                </div>
                    ...
                    {filteredNotices.map((notice, index) => (
                        ...
                        >
                            ...
                            <div className="flex-shrink-0 w-24 h-24 bg-slate-50 rounded-3xl flex flex-col items-center justify-center border border-slate-100 group-hover:bg-blue-50 transition-colors">
                                <span className="text-[#1a237e] font-black text-xl">{notice.date.split(' ')[1].replace(',', '')}</span>
                                <span className="text-blue-600 font-bold text-xs uppercase">{notice.date.split(' ')[0]}</span>
                            </div>
                            ...
                            <div className="flex-grow">
                                ...
                                <p className="text-gray-500 leading-relaxed">{notice.description}</p>
                            </div>

                            {/* Action Button */}
                            <div className="flex-shrink-0">
                                <button className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center text-[#1a237e] hover:bg-[#1a237e] hover:text-white transition-all shadow-inner">
